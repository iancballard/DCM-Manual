...
b= sub+job[:-4] #convenient name for output file that incorporates subject name and job ID, without the .mat extension
...
template_file = os.path.join(script_dir,'template_job_file_dcm.m') #this is your template .m file
temp_file = (os.path.join(script_dir) + 'template_job_file_dcm' + b + '.m') #create a temporary copy to add subject info to
...
ff = open(temp_file, 'w')
for line in f:
	line = string.replace(line, 'file', job) #in this case, the flag word is SUBJ (in the .m file), replaced with the variable subject
	line = string.replace(line, 'f_path', path)
